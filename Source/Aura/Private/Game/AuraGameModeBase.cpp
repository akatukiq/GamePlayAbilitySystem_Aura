...
void AAuraGameModeBase::DeletSlot(const FString& SlotName, int32 SlotIndex)
{
	if (UGameplayStatics::DoesSaveGameExist(SlotName, SlotIndex))
	{
...
ULoadScreenSaveGame* AAuraGameModeBase::RetriveInGameSaveData()
{
...
void AAuraGameModeBase::LoadWorldState(UWorld* World) const
{
	...
	if (UGameplayStatics::DoesSaveGameExist(AuraGI->LoadSlotName, AuraGI->LoadSlotIndex))
	{
		...
		if (SaveGame == nullptr)
		{
			UE_LOG(LogAura, Error, TEXT("Failde to load slot"));
			return;
		}
		...
		for (FActorIterator It(World); It; ++It)
		{
			...
			for (FSavedActor SavedActor: SaveGame->GetSavedMapWithMapName(WorldName).SavedActors)
			{
				if (SavedActor.ActorName == Actor->GetName())
				{
					...
					Archive.ArIsSaveGame = true;
					Actor->Serialize(Archive);// convers binary bytes back into variables
